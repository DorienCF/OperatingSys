...
#define DEBUG 1			//change this to 1 to enable verbose output Change back [ ]
...
int my_comparer(const void *this, const void *that)
{
	//TODO: IMPLEMENT ME!
	 ...
	 Process *seccond_process = (Process *) that;
	 if (first_process -> priority < seccond_process -> priority)
		{
				return 1;
		}
	else if (first_process -> priority > seccond_process -> priority)
		{
				return -1;
		}
	else
		{
			if(first_process -> arrival_time < seccond_process ->arrival_time )
				{
					return -1;
				}
			else if(first_process -> arrival_time >seccond_process ->arrival_time)
				{
					return 1;
				}
		} // Comparing first and seccond processes... GTG
	return 0;
}
