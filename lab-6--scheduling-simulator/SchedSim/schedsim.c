    ...
    for (int i = 0; i < n; i++)
      {
        ...
        {
          t = t + rem_bt[i]; // i.
          plist[i].wt = t - plist[i].bt; // ii.
          rem_bt[i] = 0; // This process is over | iii.
        } 
      }
  }
...
{ 
     ...
     *        - Calculate waiting time for each completed process. wt[i] = completion_time - arrival_time - burst_time
    ...
    while (process < n)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (rem_bt[i] > 0 && (min_time == -1 || rem_bt[i] < rem_bt[min_time]))
            {
                min_time = i;
            }
        }
        ...
        if (rem_bt[min_time] == 0)
        {
            ...
            plist[min_time].wt = completion_time - plist[min_time].art - plist[min_time].bt;
            current_time++;
            rem_bt[min_time] = INT_MAX;
        }
        else
        {
            current_time++;
...
{ 
    ...
    return 1;
      ...
      return 0; // need the other return value
} 
