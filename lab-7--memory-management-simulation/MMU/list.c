                ...
                if (current -> next == NULL) {
                        ...
                        if (newblk_size >= curblk_size) {
                                ...
                                while (current != NULL && newblk_size <= curblk_size) {
                                        ...
                                        if (current != NULL) {
                                                curblk_size = current -> blk -> end - current -> blk -> start;
                                        } // darn bracketrs are nightmares to debugg
                                }
                                prev -> next = newNode;
                                newNode -> next = current;
                        }
...
void list_coalese_nodes(list_t * l) {
        /*
         * 1. Assuming you have passed in a sorted list of blocks based on addresses in ascending order // Does this really need to be programed...
