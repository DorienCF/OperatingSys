...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy)
 {
    ...
    */
    ...
    // 1. Check if a node is in the FREE_LIST with a blk(end - start) >= blocksize
        ...
        {
            RemoveNode = list_is_in_by_size(freelist,blocksize);
        }
        //if so, remove it and go to #3, if not print ""Error: Memory Allocation <blocksize> blocks\n""
        else
        {
            printf("Error: Memory Allocation %d blocks\n",blocksize);
            
        }
        block_t* blk = list_remove_at_index(freelist, RemoveNode);
        int end = blk->end;
    // 3. set the blk.pid = pid
        blk->pid = pid; // using - > over .
    // 4. set the blk.end = blk->start + blocksize - 1
        blk->end = blk->start + blocksize - 1;
    // 5. add the blk to the ALLOC_LIST in ascending order by address.
        list_add_ascending_by_address(alloclist, blk);
    // 6.a dynamically allocate a new block_t called fragment [use malloc]
        block_t* fragment = malloc(sizeof(block_t));
    // 6.b set the fragment->pid = 0 
        fragment->pid = 0;
    // 6.c set the fragment->start = the blk.end + 1
        fragment->start =  blk->end + 1;
    // 6.d set the fragment->end = original blk.end before you changed it in #4
        fragment->end = end;
    // 6.e add the fragment to the FREE_LIST based on policy
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy)
 {
   ...
   {
    RemoveNode = list_get_index_of_by_Pid(alloclist,pid);
    blk = list_remove_at_index(alloclist,RemoveNode);
   }
   // 2.if so, remove it and go to #3, if not print "Error: Can't locate Memory Used by PID: <pid>"
    else
    {
